...
import static com.example.minesweeper.data.Board.FIELD_IS_EMPTY;
...
public class MineGenerator {
    ...
    @Getter
    private int numberOfMines;// not changeable

    public void generate() {//public method always comes as first
        setMineNumber();
        generateMines();
        calculateNumbers();
    ...
    private void setMineNumber() { //
        if (fields.length >= 5 && fields.length < 10) {
            numberOfMines = 3;
            return;
        } if (fields.length >= 5 && fields.length < 15) {
            numberOfMines = 10;
            return;
    ...
    private void generateMines() { // have to be called in specific order
    ...
    private void calculateNumbers() { // have to be called in specific order
    ...
    private void updateAdjacentFields(int i, int j) {
        ...
        int[] jValues = new int[]{1, 0, -1, 1, -1, -1, 1, -1};
